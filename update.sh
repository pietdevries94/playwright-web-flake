...
driver_file="$root/playwright-driver/driver.nix"
...
playwright_dir="$root/playwright-driver"
...
update_browser() {
	...
	replace_sha "$playwright_dir/$name.nix" "x86_64-$platform" \
		"$(prefetch_browser "https://cdn.playwright.dev/builds/$buildname/$revision/$name-$suffix.zip" $stripRoot)"
	replace_sha "$playwright_dir/$name.nix" "aarch64-$platform" \
		"$(prefetch_browser "https://cdn.playwright.dev/builds/$buildname/$revision/$name-$aarch64_suffix.zip" $stripRoot)"
}
